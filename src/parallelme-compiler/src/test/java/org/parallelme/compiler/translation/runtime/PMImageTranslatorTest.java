...
public abstract class PMImageTranslatorTest extends ImageTranslatorTest {
	...
	public void translateReduceOperation() throws Exception {
		...
		String expectedTranslation = "static float4 reduce123_func(float4 param1, float4 param2) {\n"
				...
				+ "}"
				+ "__kernel void reduce123(__global float4* PM_dataRet, __global float4* PM_tile, int PM_height) {"
		...
		expectedTranslation = String
				.format("static float4 reduce123_func(float4 param1, float4 param2, %s %s) {\n"
						...
						+ "PM_tile[PM_gid]=param1;"
						+ "}"
						+ "__kernel void reduce123(__global float4* PM_dataRet, __global float4* PM_tile, int PM_height, %s %s) {"
						+ "float4 param1 = PM_tile[0];"
						+ "float4 param2;"
						+ "for (int PM_x=1; PM_x<PM_height; ++PM_x){"
						+ "param2 = PM_tile[PM_x];"
						+ "param1 = reduce123_func(param1,param2,  %s);"
						+ "}"
						+ "*PM_dataRet = param1; }", finalVar.typeName,
		...
		ST st = new ST(
				"static float4 reduce123_func(float4 param1, float4 param2, __global <nonFinalVarType>* PM_<nonFinalVar>) {\n"
						+ "param1.s0 = 123; param2.s1=456; return param2;} \n"
						+ "__kernel void reduce123(__global float4* PM_dataRet, __global float4 *PM_data, int PM_width, int PM_height, __global <nonFinalVarType>* PM_<nonFinalVar>) {"
		...
		st = new ST(
				"static float4 reduce123_func(float4 param1, float4 param2) {\n"
						+ "param1.s0 = 123; param2.s1=456; return param2;} \n"
						+ "__kernel void reduce123(__global float4* PM_dataRet, __global float4 *PM_data, int PM_width, int PM_height) {"
		...
		st = new ST(
				"static float4 reduce123_func(float4 param1, float4 param2, <finalVarType> <finalVar>) {\n"
						+ "param1.s0 = 123; param2.s1=456; return param2;} \n"
						+ "__kernel void reduce123(__global float4* PM_dataRet, __global float4 *PM_data, int PM_width, int PM_height, <finalVarType> <finalVar>) {"
						+ "float4 param1 = PM_data[0];"
						+ "float4 param2;"
						+ "int PM_workSize = PM_height*PM_width;"
						+ "for (int PM_x=0; PM_x\\<PM_workSize; ++PM_x) {"
	...
	public void translateMapOperation() throws Exception {
		...
		ST st = new ST(
				"static <mapType> map123_func(<type> param1, int x, int y) {\n"
						...
						+ "ret = param1.s2 * 1.5f;"
						+ "return ret; } \n"
						+ "__kernel void map123(__global <mapType>* PM_dataRet, __global <type>* PM_data, int PM_width) {"
						+ "\tint PM_x = get_global_id(0);\n"
						+ "\tint PM_y = get_global_id(1);\n"
						+ "\tint PM_gid = PM_y * PM_width + PM_x;\n"
						+ "\tPM_dataRet[PM_gid] = map123_func(PM_data[PM_gid], PM_x, PM_y);"
						+ "}");
		st.add("type", getTranslatedParameterType());
		...
		st = new ST(
				"static <mapType> map123_func(<type> param1, int x, int y, <finalVarType> <finalVar>) {\n"
						...
						+ "ret = param1.s2 * 1.5f;"
						+ "return ret; } \n"
						+ "__kernel void map123(__global <mapType>* PM_dataRet, __global <type>* PM_data, int PM_width, <finalVarType> <finalVar>) {"
						+ "\tint PM_x = get_global_id(0);\n"
						+ "\tint PM_y = get_global_id(1);\n"
						+ "\tint PM_gid = PM_y * PM_width + PM_x;\n"
						+ "\tPM_dataRet[PM_gid] = map123_func(PM_data[PM_gid], PM_x, PM_y, <finalVar>);"
						+ "}");
		st.add("finalVar", finalVar.name);
		st.add("finalVarType", finalVar.typeName);
		st.add("type", getTranslatedParameterType());
		...
		st = new ST(
				"static <mapType> map123_func(<type> param1, int x, int y, __global <nonFinalVarType>* PM_<nonFinalVar>) {\n"
						...
						+ "ret = param1.s2 * 1.5f;"
						+ "return ret; } \n"
						+ "__kernel void map123(__global <mapType>* PM_dataRet, __global <type>* PM_data, int PM_width, int PM_height, __global <nonFinalVarType>* PM_<nonFinalVar>) {"
						+ "for(int PM_x=0; PM_x\\<PM_width; ++PM_x) {"
						+ "for(int PM_y=0; PM_y\\<PM_height; ++PM_y) {"
						+ "\tint PM_gid = PM_x + PM_y * PM_width;\n"
						+ "\tPM_dataRet[PM_gid] = map123_func(PM_data[PM_gid], PM_x, PM_y, PM_<nonFinalVar>);"
						+ "}}}");
		st.add("nonFinalVar", nonFinalVar.name);
		st.add("nonFinalVarType", nonFinalVar.typeName);
		st.add("type", getTranslatedParameterType());
		...
		st = new ST(
				"static <mapType> map123_func(<type> param1, int x, int y, __global <nonFinalVarType>* PM_<nonFinalVar>, <finalVarType> <finalVar>) {\n"
						...
						+ "ret = param1.s2 * 1.5f;"
						+ "return ret; } \n"
						+ "__kernel void map123(__global <mapType>* PM_dataRet, __global <type>* PM_data, int PM_width, int PM_height, "
						+ "__global <nonFinalVarType>* PM_<nonFinalVar>, <finalVarType> <finalVar>) {"
						+ "for(int PM_x=0; PM_x\\<PM_width; ++PM_x) {"
						+ "for(int PM_y=0; PM_y\\<PM_height; ++PM_y) {"
						+ "\tint PM_gid = PM_x + PM_y * PM_width;\n"
						+ "\tPM_dataRet[PM_gid] = map123_func(PM_data[PM_gid], PM_x, PM_y, PM_<nonFinalVar>, <finalVar>);"
						+ "}}}");
		st.add("nonFinalVar", nonFinalVar.name);
		st.add("nonFinalVarType", nonFinalVar.typeName);
		st.add("finalVar", finalVar.name);
	...
	public void translateFilterOperation() throws Exception {
		...
		List<String> translatedFunction = translator
				.translateOperation(operation);
		ST st = new ST(
				"static bool filter123_func(<type> param1, int x, int y) {\n"
						+ "\treturn param1.s2 > 2;\n"
						+ "}\n"
						+ "__kernel void filter123_tile(__global <type>* PM_data, __global int* PM_dataTile, int PM_width) {\n"
						+ "	int PM_x = get_global_id(0);\n"
						+ "	int PM_y = get_global_id(1);\n"
						+ "	int PM_gid = PM_y * PM_width + PM_x;\n"
						+ "	if (filter123_func(PM_data[PM_gid], PM_x, PM_y)) {\n"
						+ "		PM_dataTile[PM_gid] = PM_gid;\n"
						+ "	} else {\n"
						+ "		PM_dataTile[PM_gid] = -1;\n"
						+ "	}\n"
						+ "}\n"
						+ "__kernel void filter123(__global <type>* PM_dataRet, __global <type>* PM_data, __global int* PM_dataTile, int PM_width, int PM_height) {\n"
						+ "	int PM_count = 0;\n"
						+ "	for (int PM_x=0; PM_x\\<PM_width * PM_height; ++PM_x) {\n"
		...
		translatedFunction = translator.translateOperation(operation);
		st = new ST(
				"static bool filter123_func(<type> param1, int x, int y, <finalVarType> <finalVar>) {\n"
						+ "\treturn param1.s2 > 2;\n"
						+ "}\n"
						+ "__kernel void filter123_tile(__global <type>* PM_data, __global int* PM_dataTile, int PM_width, <finalVarType> <finalVar>) {\n"
						+ "	int PM_x = get_global_id(0);\n"
						+ "	int PM_y = get_global_id(1);\n"
						+ "	int PM_gid = PM_y * PM_width + PM_x;\n"
						+ "	if (filter123_func(PM_data[PM_gid], PM_x, PM_y, <finalVar>)) {\n"
						+ "		PM_dataTile[PM_gid] = PM_gid;\n"
						+ "	} else {\n"
						+ "		PM_dataTile[PM_gid] = -1;\n"
						+ "	}\n"
		...
		translatedFunction = translator.translateOperation(operation);
		st = new ST(
				"static bool filter123_func(<type> param1, int x, int y, __global <nonFinalVarType>* PM_<nonFinalVar>) {\n"
						+ "\treturn param1.s2 > 2;\n"
						+ "}\n"
						+ "__kernel void filter123_tile(__global <type>* PM_data, __global int* PM_dataTile, int PM_width, int PM_height, __global <nonFinalVarType>* PM_<nonFinalVar>) {\n"
						+ "	for (int PM_x=0; PM_x\\<PM_width; ++PM_x) {\n"
						+ "	for (int PM_y=0; PM_y\\<PM_height; ++PM_y) {\n"
						+ "	int PM_gid = PM_x +  PM_width * PM_y;\n"
						+ "	if (filter123_func(PM_data[PM_gid], PM_x, PM_y, PM_<nonFinalVar>)) {\n"
						+ "		PM_dataTile[PM_gid] = PM_gid;\n"
						+ "	} else {\n"
						+ "		PM_dataTile[PM_gid] = -1;\n"
						+ "	}\n"
		...
		translatedFunction = translator.translateOperation(operation);
		st = new ST(
				"static bool filter123_func(<type> param1, int x, int y, __global <nonFinalVarType>* PM_<nonFinalVar>, <finalVarType> <finalVar>) {\n"
						+ "\treturn param1.s2 > 2;\n"
						+ "}\n"
						+ "__kernel void filter123_tile(__global <type>* PM_data, __global int* PM_dataTile, int PM_width, int PM_height, "
						+ "		__global <nonFinalVarType>* PM_<nonFinalVar>, <finalVarType> <finalVar>) {\n"
						+ "	for (int PM_x=0; PM_x\\<PM_width; ++PM_x) {\n"
						+ "	for (int PM_y=0; PM_y\\<PM_height; ++PM_y) {\n"
						+ "	int PM_gid = PM_x +  PM_width * PM_y;\n"
						+ "	if (filter123_func(PM_data[PM_gid], PM_x, PM_y, PM_<nonFinalVar>, <finalVar>)) {\n"
						+ "		PM_dataTile[PM_gid] = PM_gid;\n"
						+ "	} else {\n"
						+ "		PM_dataTile[PM_gid] = -1;\n"
						+ "	}\n"
	...
	public void translateFilterOperationJNI() throws Exception {
		...
		ST st = new ST(
				"JNIEXPORT jlong JNICALL Java_SomeClass_filter123(JNIEnv *env, jobject self, jlong PM_runtime, jlong PM_data) {\n"
						...
						+ "auto PM_dataPtr = (ImageData *) PM_data;\n"
						+ "auto PM_task = std::make_unique\\<Task>(PM_runtimePtr->program);\n"
						+ "auto PM_tileBuffer = std::make_shared\\<Buffer>(sizeof(int) * PM_dataPtr->width*PM_dataPtr->height);\n"
						...
						+ "PM_runtimePtr->runtime->submitTask(std::move(PM_task));\n"
						+ "PM_runtimePtr->runtime->finish();\n"
						+ "jintArray PM_tileArray = env->NewIntArray(PM_dataPtr->width * PM_dataPtr->height);\n"
						...
						+ "->setArg(3, PM_dataPtr->width)\n"
						+ "->setArg(4, PM_dataPtr->height)\n"
						+ "->setWorkSize(1);\n"
						+ "});\n"
