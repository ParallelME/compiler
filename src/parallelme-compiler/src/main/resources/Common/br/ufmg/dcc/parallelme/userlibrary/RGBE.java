...
public class RGBE {
    ...
    public static ResourceData loadFromResource(Resources res, int resource) {
        ...
        try {
            DataInputStream in = new DataInputStream(new BufferedInputStream(res.openRawResource(resource)));
            RGBE.Header header = RGBE.readHeader(in);
            resData.width = header.getWidth();
            resData.height = header.getHeight();

            resData.data = new byte[resData.width * resData.height * 4];
            RGBE.readPixelsRawRLE(in, resData.data, 0, resData.width, resData.height);
